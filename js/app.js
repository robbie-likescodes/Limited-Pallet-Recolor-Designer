// js/app.js — Orchestrator (fixed helper signatures)
// Uses your modular tree; renders + maps with HEIC-safe loading.
...
const els = {
  ...
  // halftone (optional toggles in your UI)
...
function setPaletteFromHexes(hexes = [], tol=64){
  ...
  // by default, preselect all as restricted; user can pare down
  const all = new Set(hexes.map((_,i)=>i));
  state.restricted = all;
  renderRestrictedFromPalette(els, hexes, all);
  renderCodeList();
  updateMailto();
  persistPrefs();
}
...
function drawPreviewFromState(){
  ...
  // FIX: getOrientedDims expects (orientation, w, h, [maxW])
  const { w, h } = getOrientedDims(state.exifOrientation, state.fullW, state.fullH, maxW);
  if (!w || !h) return;

  els.srcCanvas.width = w; els.srcCanvas.height = h;
  sctx.save(); sctx.fillStyle = '#0b172e'; sctx.fillRect(0,0,w,h); sctx.restore();
  // FIX: drawImageWithOrientation(ctx, img, w, h, orientation)
...
async function saveCurrentProject(){
  ...
  // FIX: getOrientedDims expects (orientation, w, h, maxRef)
  const {w:ow,h:oh}=getOrientedDims(o, state.fullW, state.fullH, Math.max(state.fullW, state.fullH));
  ...
  // FIX: drawImageWithOrientation(ctx, img, w, h, orientation)
  drawImageWithOrientation(tc, state.fullBitmap, ow, oh, o);
...
function applySettings(s){
  ...
  state.regions.length=0; // (If your lasso module persists masks, rebuild here.)
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // restricted all/none
  on(els.restrictedSelectAll,'click', ()=>{ $$('input[type=checkbox]', els.restrictedList).forEach(c=>c.checked=true); state.restricted=new Set(getRestrictedInkIndices()); renderCodeList(); updateMailto(); });
  on(els.restrictedSelectNone,'click', ()=>{ $$('input[type=checkbox]', els.restrictedList).forEach(c=>c.checked=false); state.restricted=new Set(getRestrictedInkIndices()); renderCodeList(); updateMailto(); });
  ...
  // editor overlay (optional module)
